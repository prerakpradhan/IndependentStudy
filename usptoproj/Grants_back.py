    ...
    for filePath in fileList:
        ...
        if(formatStr=='aps'):
            ...
            try:
                ...
                print "here and now parsing xml" +filePath
                g=GrantsParser()
                g.extractXML4(xmlStr)
                print "done done done with parsing writing now"
                g.writeCSV()
...
def printfile(filelist):
    n = 5;
    m = len(filelist)
    firstlist = filelist[0:m/n]
    secondlist = filelist[m/n:2*m/n]
    thirdlist = filelist[2*m/n:3*m/n]
    fourthlist= filelist[3*m/n:4*m/n]
    fifthlist = filelist[4*m/n:5*m/n]
...
if __name__=="__main__":
    ...
    #processes.append(multiprocessing.Process(target=mainProcess,args=(ls_xml4_p[0],'xml4')))
    #for p in processes:
      #  p.start()
    #for p in processes:
      #  p.join()
